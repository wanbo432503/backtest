...
def test_value_quality_strategy_uses_mocked_fundamentals_when_available():
    strategy = get_selection_strategy("value_quality")
    same = build_ohlcv_frame(periods=180, daily_return=0.001, base_volume=1_200_000)
    ...
    rows = score_candidates_with_strategy(
        {"SH603019": same.copy(), "SZ002241": same.copy()},
        pd.Timestamp("2025-09-15"),
        SelectionConfig(top_n=1, min_history_bars=100),
        strategy,
    ...
    assert rows[0]["symbol"] == "SH603019"
    assert rows[0]["strategy_factor_values"]["roe"] == 0.22
    assert rows[0]["normalized_strategy_factors"]["roe"] > rows[1]["normalized_strategy_factors"]["roe"]


def test_value_quality_strategy_marks_missing_fundamentals_without_crashing():
    strategy = get_selection_strategy("value_quality")
    same = build_ohlcv_frame(periods=180, daily_return=0.001, base_volume=1_200_000)
    ...
    rows = score_candidates_with_strategy(
        ...
        SelectionConfig(top_n=1, min_history_bars=100),
        strategy,
        fundamentals_by_symbol={"SH603019": {"roe": 0.2}},
    )

    assert len(rows) == 2
    assert all(row["skip_reason"] is None for row in rows)
    assert any("missing_fundamentals" in row["warnings"] for row in rows)
