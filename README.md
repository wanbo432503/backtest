...
- Phase 3.0 组合选股回测：支持最多 4 只 `60` / `00` 开头股票，按因子选 Top N，并按周、双周或月度调仓。
...
Phase 3.0 面向“选股 + 组合交易 + 周期性调仓”的中低频原型。页面左侧的主入口是“组合选股回测”，默认股票池为 `SH603019` 和 `SZ002241`。

支持范围：

- 股票池最多 4 只。
- 仅支持 `60` / `00` 开头股票，例如 `SH603019`、`SZ002241`。
...
1. 在股票池中输入 1-4 只允许交易的股票。
2. 设置 Top N、调仓周期、因子权重和组合风控。
3. 点击“开始组合回测”。
4. 查看组合摘要、净值曲线、持仓、调仓、候选、成交和提示页签。
5. 点击持仓或候选股票代码，可同步到右侧 TradingAgents 智能分析标的。
