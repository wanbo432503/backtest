...
Backtest 是一个面向 A 股代码的量化回测 Web 应用，基于 FastAPI、Pandas、Bokeh 和免费公开数据源构建。当前产品口径只支持 A 股代码，不再支持美股、港股或加密货币作为回测对象。
...
- 多股票信号组合回测：统一策略库中的七个单股策略均可用于多股票组合；每只股票独立产生信号，组合统一管理共享现金、持仓上限、总敞口、回撤门控和市场宽度过滤。
